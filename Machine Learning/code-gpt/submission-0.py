...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        b, t = context.shape
        pos = torch.arange(0, t, device=context.device)
        tok_emb = self.embedding(context) # (b, t, model_dim)
        pos_emb = self.pe(pos)
        x = pos_emb+tok_emb
        x = self.mhattn(x)
        return torch.round(nn.functional.softmax(self.proj(self.norm1(x)),dim=2),decimals=4)
